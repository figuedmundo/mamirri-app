...
export function ComparacionBoard({
  ...
  const [activeTab, setActiveTab] = React.useState<
    'huellas' | 'postura' | 'tests'
  >('huellas');

  const initialHuella = casoClinico.evaluacion.huellas.find(
    (h) => h.tipo === 'inicial',
  );
  const finalHuella = casoClinico.evaluacion.huellas.find(
    (h) => h.tipo === 'final',
  );

  const initialVideo = casoClinico.evaluacion.videosPostura[0];
  const finalVideo =
    casoClinico.evaluacion.videosPostura[
      casoClinico.evaluacion.videosPostura.length - 1
    ];
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-medium text-slate-400 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded">
            {itemInitial?.fecha
              ? new Date(itemInitial.fecha).toLocaleDateString()
              : 'Sin datos'}
          </span>
        </div>
        ...
        {itemInitial?.analisis && (
          <div className="text-sm text-slate-600 dark:text-slate-300">
            <p>
              ...
              {itemInitial.analisis.arco}
            </p>
            <p>
              <span className="font-medium">Desviación:</span>{' '}
              {itemInitial.analisis.desviacion}
            </p>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-medium text-teal-600 dark:text-teal-400 bg-teal-50 dark:bg-teal-900/20 px-2 py-1 rounded">
            {itemFinal?.fecha
              ? new Date(itemFinal.fecha).toLocaleDateString()
              : 'Pendiente'}
          </span>
        </div>
        ...
        {itemFinal?.analisis && (
          <div className="text-sm text-slate-600 dark:text-slate-300">
            <p>
              <span className="font-medium">Arco:</span>{' '}
              {itemFinal.analisis.arco}
            </p>
            <p>
              <span className="font-medium">Desviación:</span>{' '}
              {itemFinal.analisis.desviacion}
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      <div className="min-h-[400px]">
        {activeTab === 'huellas' &&
          renderComparisonCard(initialHuella, finalHuella, (item: Huella) => (
            <img
              src={item.url}
              alt="Huella"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="text-2xl font-bold text-emerald-500 mt-1">
                  {casoClinico.sesionesTratamiento[
                    casoClinico.sesionesTratamiento.length - 1
                  ]?.dolorFinal ?? '-'}
                  /10
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700">
                ...
                <div className="text-2xl font-bold text-slate-800 dark:text-slate-100 mt-1">
                  {Math.round(
                    (new Date().getTime() -
                      new Date(casoClinico.fechaInicio).getTime()) /
                      (1000 * 60 * 60 * 24 * 7),
                  )}{' '}
                  sem
                </div>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                  <tr>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                      {Math.min(
                        12,
                        casoClinico.evaluacion.evaluacionAVD.barthel.total + 2,
                      )}
                      /12
                    </td>
                  ...
                  <tr>
                    ...
                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                      {casoClinico.evaluacion.testOrtopedicos.schober.resultado}{' '}
                      cm
                    </td>
                    <td className="px-6 py-4 text-sm text-slate-600 dark:text-slate-300">
                      {casoClinico.evaluacion.testOrtopedicos.schober
                        .resultado + 2}{' '}
                      cm
                    </td>
                    <td className="px-6 py-4 text-sm text-emerald-600 font-medium">
      ...
      {finalHuella?.comparacion && (
        <div className="bg-teal-50 dark:bg-teal-900/10 rounded-xl p-6 border border-teal-100 dark:border-teal-800/50">
          ...
          <p className="text-teal-800 dark:text-teal-200 mb-4">
            {finalHuella.comparacion.mejoraDolor}.{' '}
            {finalHuella.comparacion.recuperacionROM} de rango de movimiento.
          </p>
          <div className="flex gap-2">
