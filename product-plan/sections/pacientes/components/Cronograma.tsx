...
export function Cronograma({
  ...
  const sortedSessions = [...sesiones].sort((a, b) => 
    new Date(b.fecha).getTime() - new Date(a.fecha).getTime()
  )

  const getPainColor = (level: number) => {
    if (level <= 3) return 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300'
    if (level <= 6) return 'bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-300'
      ...
      <div className="relative pl-8 border-l-2 border-slate-200 dark:border-slate-700 space-y-8">
        {sortedSessions.length === 0 ? (
          <div className="text-center py-12 bg-slate-50 dark:bg-slate-800/50 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700">
            <p className="text-slate-500 dark:text-slate-400">No hay sesiones registradas.</p>
            <button
              onClick={onAddSession}
          ...
          sortedSessions.map((sesion) => (
            <div key={sesion.id} className="relative">
              ...
              <div 
                ...
                onClick={() => onViewSession?.(sesion.id)}
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-xs font-medium text-teal-600 dark:text-teal-400 uppercase tracking-wider bg-teal-50 dark:bg-teal-900/20 px-2 py-1 rounded">
                      Fase {sesion.faseNumero}
                    </span>
                  </div>
                  
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button 
                      onClick={(e) => { e.stopPropagation(); onEditSession?.(sesion.id) }}
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {sesion.tecnicasAplicadas.map((tecnica, idx) => (
                        <span 
                          key={idx} 
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">
                        "{sesion.respuestaPaciente}"
                      </p>
                      <div className={`flex-shrink-0 flex flex-col items-center justify-center w-10 h-10 rounded-lg ${getPainColor(sesion.dolorFinal)}`}>
                        <span className="text-sm font-bold">{sesion.dolorFinal}</span>
                        <span className="text-[10px] uppercase font-bold opacity-70">END</span>
                      </div>
                    </div>
                  </div>
                </div>
