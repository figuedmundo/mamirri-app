...
export function CategoryNav({ categorias, selectedCategoryId, onSelectCategory }: CategoryNavProps) {
  return (
        ...
        return (
          ...
          >
            <div className={clsx(
              ...
              isSelected ? "bg-teal-100 text-teal-700 dark:bg-teal-800 dark:text-teal-200" : "bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400"
            )}>
              <Icon size={20} strokeWidth={2} />
            </div>
            <span className="text-xs font-medium text-center leading-tight">{cat.nombre}</span>
          </button>
        )
      })}
    </div>
